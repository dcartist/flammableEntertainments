...
class EpisodeMain extends Component {
    ...
    componentDidMount(){
        axios.get(`https://flameableent.com/wp-json/acf/v3/posts/${this.state.id}`).then(results=>(
            this.setState({episodeData: results.data})
        ))
    }
    ...
    render() {
        if (this.state.episodeData.length == 0){
            ...
            if (this.state.episodeData.acf.episode_num == null){
                ...
                return (
                    <div>
                        <Episode {...this.state.episodeData.acf}></Episode>
                </div>
            );
            }
        }
